...
function App() {
  ...
  const[date] = useState(today.getFullYear() + '-' +  ('0' + (today.getMonth() + 1)).slice(-2) + '-' + ('0' + today.getDate()).slice(-2));

  const[time] = useState(("0" + (today.getHours())).slice(-2) + ':' + ('0' + today.getMinutes()).slice(-2) + ':' + ('0' + today.getSeconds()).slice(-2));
  ...
  const ascendingProvinces = () =>{
    const sorted = [...CanadaProvincesConfirmed].sort((a,b) => {return b.Confirmed - a.Confirmed});
    setCanadaConfirmed(sorted);
  }

  const descendingProvinces = () =>{
    const sorted = [...CanadaProvincesConfirmed].sort((a,b) => {return b.Confirmed - a.Confirmed}).reverse();
    setCanadaConfirmed(sorted);
  }

  const ascendingStates = () =>{
    const sorted = [...usStatesConfirmed].sort((a,b) => {return b.Confirmed - a.Confirmed});
    setUSConfirmed(sorted);
  }

  const descendingStates = () =>{
    const sorted = [...usStatesConfirmed].sort((a,b) => {return b.Confirmed - a.Confirmed}).reverse();
    setUSConfirmed(sorted);
  }
  ...
  return (
    <div className="body-nav">
      ...
      <div className = "body" style={{paddingLeft:'3em'}}>
          ...
          <div style={{width: 'auto', height: 'auto', paddingLeft: '3em', paddingTop:'3em', paddingRight: '3em'}}>
              <ReactMapGl
                        ...
                        if(province.Date.includes(date) && province.Province != "Recovered"){
                          return(
                              <tr key={index}>
                                ...
                                <td style={{color: (province.Confirmed >= 100) ? "#FFC300":""}}>{province.Confirmed}</td>
